    ...
    testList "Sudoku CrossGroup tests" [
        ...
        test "2 by 2 corner group" {
            Expect.isTrue (CrossGroup.groupsAtLevel 2 2 1 |> groupsContain {
                Level = 1
                Intersections = seq { seq { for col in 0..1 -> (0, col)}}
                Source = seq [(0, 2); (0, 3)]
                Target = seq [(1, 0); (1, 1)]
            }) "first row"
        }
        ...
        test "3 by 4 level 1 corner group" {
            Expect.isTrue (CrossGroup.groupsAtLevel 3 4 1 |> groupsContain {
                Level = 1
                Intersections = seq { seq { for col in 0..2 -> (0, col)}}
        ...
        test "4 by 3 level 1 corner group" {
            Expect.isTrue (CrossGroup.groupsAtLevel 4 3 1 |> groupsContain {
                Level = 1
                Intersections = seq { seq { for col in 0..3 -> (0, col)}}
        ...
        test "4 by 3 level 1 last corner row group" {
            Expect.isTrue (CrossGroup.groupsAtLevel 4 3 1 |> groupsContain {
                Level = 1
                Intersections = seq { seq { for col in 8..11 -> (11, col)}}
        ...
        test "4 by 3 level 1 last corner column group" {
            Expect.isTrue (CrossGroup.groupsAtLevel 4 3 1 |> groupsContain {
                Level = 1
                Intersections = seq { seq { for row in 9..11 -> (row, 11)}}
        ...
        test "4 by 4 level 2 corner group" {
            Expect.isTrue (CrossGroup.groupsAtLevel 4 4 2 |> groupsContain {
                Level = 2
                Intersections = seq {
                    seq { for col in 0..3 do for row in 0..1 -> (row, col)}
                    seq { for col in 4..7 do for row in 0..1 -> (row, col)}
                }
                Source = seq { for row in 0..1 do for col in 8..15 -> (row, col)}
